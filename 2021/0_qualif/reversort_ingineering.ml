...
let solve _ =
  let (size, cost) = Read.(line (pair int)) in
  let a = Array.init size ((+) 1) in
  let rec reverse i j =
    if i >= j then ()
    else
      let ai = a.(i - 1) in
  ...
  let rec solve size cost =
    if size <= 1 then
      ...
      (
        let target_cost = cost - size + 2 in
        let chosen_cost = max 1 (min target_cost size) in
        reverse 1 chosen_cost;
        solve (size - 1) (cost - chosen_cost)
      )
  in
  solve size cost

let () = iterate_cases solve Write.(impossible_or (array int))
