...
let solve _ =
  let (x, y, s) = Read.(line (tripleg int int string)) in
  let first_char =
    match String.index_opt s 'C', String.index_opt s 'J' with
    | None, None -> 'C'
    | Some _, None -> 'C'
    | None, Some _ -> 'J'
    | Some i, Some j -> if i < j then 'C' else 'J'
  in
  let prev_char = ref first_char in
  let cost = ref 0 in
  String.iter
    (fun c ->
       (
         match c with
         | 'C' when !prev_char = 'J' -> cost := !cost + y
         | 'J' when !prev_char = 'C' -> cost := !cost + x
         | _ -> ()
       );
       if c <> '?' then
         prev_char := c)
