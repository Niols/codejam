...
let solve _ =
  let player_to_questions =
    List.init 100
      (fun _ ->
         let l = Read.(line string) in
         List.init 10000 (fun i -> l.[i] = '1'))
  in
  let a = Array.make 10000 100 in
  List.iter
    (List.iteri (fun i b -> if b then a.(i) <- a.(i) - 1))
    player_to_questions;
  let points =
    List.mapi
      (fun i l ->
         let points = ref 0 in
         List.iteri
           (fun i b -> if b then points := !points + a.(i))
           l;
         (i, !points))
      player_to_questions
  in
  let rec max_points best value = function
    | [] -> best
    | (i, p) :: others ->
      if p > value then
        max_points i p others
      else
        max_points best value others
  in
  1 + max_points (-1) 0 points

let () = iterate_cases solve Write.int
