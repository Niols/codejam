...
let solve _ =
  ...
  let l = Read.(line (array int)) in
  assert (Array.length l = n);
  let position_of_minimum_element_from i =
    let rec position_of_minimum_element_from i min pos =
      if i > n then
        pos
      else if l.(i - 1) < min then
        position_of_minimum_element_from (i + 1) l.(i - 1) i
      else
        position_of_minimum_element_from (i + 1) min pos
    in
    position_of_minimum_element_from (i + 1) l.(i - 1) i
  in
  let rec reverse i j =
  ...
  in
  for i = 1 to n - 1 do
    let j = position_of_minimum_element_from i in
    reverse i j
  done;
  !cost
