...
let solve fmt s =
  ...
  for i = 0 to String.length s - 1 do
    let n = int_of_string (String.make 1 s.[i]) in
    while n > !par_level do open_par () done;
    while n < !par_level do close_par () done;
    Format.pp_print_int fmt n
  done;
  while !par_level > 0 do close_par () done

let () =
  let t = Read.(line1 int) in
  for x = 1 to t do
    Format.printf "Case #%d: %a@." x solve (read_line ())
  done
