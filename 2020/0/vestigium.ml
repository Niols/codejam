...
let row_has_duplicate n s m i =
  let r = ref false in
  Array.fill s 0 n false;
  for j = 0 to n - 1 do
    let e = m.(i).(j) - 1 in
    if s.(e) then
      r := true
...
let solve m =
  ...
  for i = 0 to n - 1 do
    r := !r + (if row_has_duplicate n s m i then 1 else 0)
  done;
  for j = 0 to n - 1 do
    c := !c + (if col_has_duplicate n s m j then 1 else 0)
  done;
  (!k, !r, !c)
...
let () =
  ...
  for x = 1 to t do
    ...
    for i = 0 to n-1 do
      m.(i) <- Read.(line_a int);
      assert (Array.length m.(i) = n)
    done;
    let (k, r, c) = solve m in
    Format.printf "Case #%d: %d %d %d@." x k r c
  done
