...
let rec solve_pos n jumps x y =
  if x = 0 && y = 0 then
    Some jumps
  else
    (
      let x0, x1, x1plus = (x land 1), (x land 2) / 2, x / 2 in
      let y0, y1, y1plus = (y land 1), (y land 2) / 2, y / 2 in

      if x0 = y0 then
        None
      else
        (
          let jump, x1plus, y1plus =
            if x1 <> y1 || (x1plus = 0 && y1plus = 0) then
              (if x0 = 1 then ('E', x1plus, y1plus) else ('N', x1plus, y1plus))
            else
              (if x0 = 1 then ('W', x1plus+1, y1plus) else ('S', x1plus, y1plus+1))
          in
          solve_pos (2 * n) (jump :: jumps) x1plus y1plus
        )
    )

let solve x y =
  let xpos, x = if x > 0 then (true, x) else (false, -x) in
  let ypos, y = if y > 0 then (true, y) else (false, -y) in
  match solve_pos 1 [] x y with
